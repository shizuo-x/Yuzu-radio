...
logger = logging.getLogger('discord_bot.ai_backends.deepseek')

class DeepSeekBackend:
    def __init__(self, bot):
        self.bot = bot
        if config.DEEPSEEK_API_KEY:
            logger.info(f"DeepSeek Backend Initialized (Model: {config.DEEPSEEK_MODEL})")
        else:
            logger.warning("DEEPSEEK_API_KEY not set.")

    def get_tools_schema(self):
        """Defines the OpenAI-compatible tool schema for DeepSeek."""
        return [
            {
                "type": "function",
                "function": {
    ...
    async def generate_response(self, history_tuples, user_prompt, system_instructions, tool_executor):
        """
        Generates a response using OpenAI-compatible API, handling tool calls.
        """
        ...
        try:
            async with self.bot.http_session.post(config.DEEPSEEK_API_URL, headers=headers, json=payload, timeout=60) as response:
                if response.status != 200:
                    error_text = await response.text()
                    logger.error(f"DeepSeek API Error: {response.status} - {error_text}")
                    return f"I encountered an error with my brain ({response.status})."
                ...
                if message.get('tool_calls'):
                    ...
                    # Add the assistant's request to messages history (required by OpenAI API spec)
                    ...
                    for tool_call in tool_calls:
                        ...
                        logger.info(f"DeepSeek requested tool: {function_name}")
                    ...
                    # We can remove tools from the second request if we want to force text, 
                    # but usually keeping them is fine. Some models might try to loop.
                    # Let's keep them for consistency.
                    
                    async with self.bot.http_session.post(config.DEEPSEEK_API_URL, headers=headers, json=payload, timeout=60) as response2:
            ...
            logger.exception(f"DeepSeek Execution Error: {e}")
